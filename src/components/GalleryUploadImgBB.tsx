...
interface GalleryUploadImgBBProps {
  images: string[];
...
export function GalleryUploadImgBB({ images, onChange, label, maxImages = 40 }: GalleryUploadImgBBProps) {
  ...
  const handleMultipleFileUpload = async (files: FileList) => {
    ...
    const validFiles = Array.from(files).filter(file => {
      ...
      const isValidSize = file.size <= 32 * 1024 * 1024; // 32MB
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Upload automático com ImgBB - Múltiplos arquivos */}
      <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 bg-gray-50">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <Cloud className="h-5 w-5 text-primary-500" />
            <span className="text-sm font-medium text-gray-700">
              Upload Automático com ImgBB (Múltiplo)
            </span>
          </div>
          ...
          {isUploading ? (
            ...
            <>
              ...
              <p className="text-xs text-gray-400 mb-2">
                JPEG, PNG, WEBP, GIF (máx. 32MB cada)
              </p>
        ...
        <p className="text-xs text-gray-500 mt-2">
          Upload gratuito e ilimitado • As imagens ficam hospedadas na nuvem • Processamento em lote
        </p>
      </div>
